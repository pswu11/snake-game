...
// // implement gameloop using concept of framerate 
...
//   const speed = 200 - (snake.length * 10 > 140 ? 140 : snake.length * 10)
//   if (elapsed > speed) {
//     moveSnake(currentDirection)
//     then = now // 10
//   }
...
// // current time (of the loop) - last time (when the loop run)

// // prevTime = 2, currTime = 3 -> delta = 1 -> 1 second has passed
// // speed = 1 block per second -> moveDiff = 1 block / second * 1 second = 1 block
// // speed = 0.5 -> moveDiff = 0.5 block / second * 1 second = 0.5 blocks
// // => move by floor(moveDiff)